...
class BST:
    ...
    def deep_pre_order(self, root, all_nodes=[]):
        if root is not None:
            all_nodes.append(root)
            self.deep_pre_order(root.LeftChild, all_nodes)
            self.deep_pre_order(root.RightChild, all_nodes)
        return all_nodes

    def deep_in_order(self, root, all_nodes=[]):
        if root is not None:
            self.deep_in_order(root.LeftChild, all_nodes)
            all_nodes.append(root)
            self.deep_in_order(root.RightChild, all_nodes)
        return all_nodes

    def deep_post_order(self, root, all_nodes=[]):
        if root is not None:
            self.deep_post_order(root.LeftChild, all_nodes)
            self.deep_post_order(root.RightChild, all_nodes)
            all_nodes.append(root)
        return all_nodes

    def wide_nodes(self, root_nodes, all_nodes=[]):
        new_root_nodes = []
        ...
        if len(root_nodes):
            for root_node in root_nodes:
                if root_node is not None:
                    ...
                    new_root_nodes.append(root_node.LeftChild)
                    new_root_nodes.append(root_node.RightChild)
            self.wide_nodes(new_root_nodes, all_nodes)
    ...
    def DeepAllNodes(self, mode):
        all_nodes = []
        if mode == 0:
            all_nodes = self.deep_in_order(self.Root)
        if mode == 1:
            all_nodes = self.deep_post_order(self.Root)
        if mode == 2:
            all_nodes = self.deep_pre_order(self.Root)
        return tuple(all_nodes)

    def WideAllNodes(self):
        all_nodes = self.wide_nodes([self.Root])
        return tuple(all_nodes)
